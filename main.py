...
def get_small_window(array_data,raw_array_data,LEN):
...
def result():
    ...
    # 读取参数
    win_len = request.form.get('win_len')
    ...
    f.save(file_path)
    array_data = pd.read_csv(file_path, header=None).values.astype(np.float32)[:,:-1]
    array_data_copy = array_data.copy()
    array_data_copy[array_data_copy<TH]=0
    # 计算结果
    
    data, h_line_intensity, v_line_intensity,center_x,center_y = get_small_window(array_data_copy,array_data,LEN)
    ...
    # 保存图片并返回结果
    output_path = os.path.join('static', f'{os.path.splitext(f.filename)[0]}.png')
    plt.savefig(output_path)
    result_url = url_for('static', filename=f'{os.path.splitext(f.filename)[0]}.png')
    plt.close(fig)
